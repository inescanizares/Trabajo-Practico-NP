...
significativa entre los grupos. Se concluye que se rechaza la hipótesis
nula de igualdad de medianas, esto confirma que las condiciones de
cultivo y la inclusión de los compuestos antifibrogénicos modifican de
manera significativa la tendencia central de la actividad fibrogénica de
las HSC.

Para identificar qué tratamientos difieren entre sí después de rechazar
la hipótesis nula de Kruskal-Wallis, se realizaron comparaciones
múltiples por pares mediante la prueba de Dunn con ajuste de Holm. El
objetivo fue determinar cuáles pares de condiciones presentaban una
mediana de producción de colágeno significativamente diferente.
...
\item
  Regla de decisión:
  \(\text{Rechazo } H_0 \text{ si} \ L \ge L_{1 - \alpha}\) Dado que el
  p-value \textgreater{} 0.05 \(\implies\) No Rechazo \(H_0\) en los 4
  test realizados.
\end{itemize}
...
\item
  ...
  Dado que el p-value \textless{} 0.05 \(\implies\) Rechazo \(H_0\).
\end{itemize}
...
\subsection{Comparación de tratamientos con el grupo control (SFB1\%)
por pares
...
\item
  ...
  Dado que el p-value \textless{} 0.05 para \(|R_1 - R_2|\) y
  \(|R_1 - R_4|\) \(\implies\) Rechazo \(H_0\)

  Dado que el p-value \textgreater{} 0.05 para \(|R_1 - R_3|\)
  \(\implies\) No Rechazo \(H_0\)
\end{itemize}
